...
<template>
  <ul v-if="loaded === 1 && outputs" class="py-3">
    <li v-for="(k, v) in nameMap" aria-label="win" class="p-4 flex" :key="v">
      ...
      <div class="space-x-2 flex text-nowrap my-auto w-fit float-end">
        ...
        <button v-if="v === 'android-universal' && isPC" class="hover:bg-slate-600 border-2 border-white rounded px-3 py-2 text-sm text-slate-300" @click="showQR = !showQR">
          扫码下载
        </button>
      </div>
